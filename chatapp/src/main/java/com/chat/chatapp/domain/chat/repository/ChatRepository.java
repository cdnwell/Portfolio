...
public class ChatRepository {
    ...
    // 채팅방 유저 리스트에 유저 추가
    public String addUser(String roomId, String userNmae) {
        ChatRoom chatRoom = chatRoomMap.get(roomId);
        String userUUID = UUID.randomUUID().toString();
        // 아이디 중복 확인 후 userList에 추가
        chatRoom.getUserList().put(userUUID, userNmae);
    ...
    public String isDuplicateName(String roomId, String username) {
        ...
        // 이 때 랜덤한 숫자를 붙였을 때,
        // getUserList 안에 있는 닉네임이라면 다시 랜덤한 숫자 붙이기.
        while(chatRoom.getUserList().containsValue(temp)) {
            int randNum = (int) (Math.random() * 100) + 1;
            temp = username+randNum;
        }
